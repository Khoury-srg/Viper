...
from cobraplus_backend.cobraplus.utils.utils import load_logs
from ..parse.wr_range_parse import construct_all_txns
from ..parse.wr_range_parse import construct_single_txn4wrrange
...
from ..utils.range_utils import ext_index, ext_writes_fn, ext_reads_fn, \
    ...
    k_writes_fn, Tk_set_fn, is_null, NULL_VALUE, in_between, k_upserts_fn
...
def construct_graph_from_log(logs_folder:str, graphs_folder:str, analysis_folder:str,
    ...
    full_history, histories = construct_all_txns(normal_logs, construct_single_txn4wrrange)
    if hasFinal:
        final_txn = construct_single_txn4wrrange(final_txn_log, len(full_history))
        full_history.append(final_txn)
    ...
    for query_txn_index, query_mop_index in queries:
        ...
        k1, k2, query_result, dead_result, isFinal = query[1:]
        ...
        for k in other_keys:  # this query must be before any upsert of k
            ...
            for ins_pos in k_upserts_pos:
                ...
                if query_txn_index == ins_pos[0]:
                    assert query_mop_index < ins_pos[1]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--analysis-dir", type=str, default="resources/Analysis_logs",
                        help="the folder of all the graph files")
    parser.add_argument("--table-count", type=int, help="how many tables in your test workload")
