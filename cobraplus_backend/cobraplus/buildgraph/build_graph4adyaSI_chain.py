            ...
            for read_txn in read_txns:
                if read_txn in k_writes: # if this read txn also writes this key
                    if len(write_txns) == 0:
                        ...
                        assert (k, write_txns[0]) not in wwpairs, \
                            f"txn {write_txns[0]} k-preceds {wwpairs[(k, write_txns[0])]}, can't k-precedes {read_txn} simultaneously"
